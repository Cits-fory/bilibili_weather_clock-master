...
//  String url = "https://api.seniverse.com/v3/weather/now.json?key=Sed_lnwUBFUn2yVth&location=shenzhen&language=zh-Hans&unit=c";
...
//        Serial.println(payload);
        ...
        if(jsonBuffer.as<String>())
        {
          ...
          String cond_txt = root["now"]["cloud"];
...
void HeFeng::doUpdateFore(HeFengForeData *data, String key, String location) {  //获取预报
  ...
  client->setInsecure();
  HTTPClient https;
  String url = "https://devapi.qweather.com/v7/weather/3d?lang=en&location=" + location + "&key=" + key + "&gzip=n";
